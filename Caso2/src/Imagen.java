...
* */
    public Imagen (String input) {
        nombre = new String(input); try {
            FileInputStream fis = new FileInputStream(nombre); fis.read(header);
            ...
            System.out.println("Ancho: " + ancho + " px, Alto: " + alto + " px"); imagen = new byte[alto][ancho][3];
    ...
    */
    private void escribirBits(int contador, int valor, int numbits) {
        ...
        for (int i=0; i < numbits ; i++) {
            ...
            int fila = (8*contador + i) / bytesPorFila;// Cada posición de la matriz agrupa 3 bytes (RGB). 
            int col = ((8*contador + i) % bytesPorFila) / 3; 
            int color = ((8*contador + i) % bytesPorFila) % 3;
            mascara = valor >> i; mascara = mascara & 1;
    ...
    */
    public void esconder( char [] mensaje, int longitud) { int contador=0;
        ...
        for (int i=0; i < longitud; i ++) { 
            ...
            escribirBits(contador, elByte, 8) ; contador++;
            if ( i % 1000 == 0) {
    ...
    */
    public void escribirImagen(String output) { 
        byte pad = 0;
        try {
            FileOutputStream fos = new FileOutputStream(output); fos.write(header);
            byte[] pixel = new byte[3];
            for (int i = 0; i < alto; i++) {
                for (int j = 0; j < ancho; j++) {
                    // Leer los 3 bytes del píxel (B, G, R) 
    ...
    */
    public void recuperar(char[] cadena, int longitud ) {
        ...
        for (int posCaracter = 0; posCaracter < longitud; posCaracter++) { cadena[posCaracter]=0;
            for (int i=0; i < 8 ; i++) {
                // los dos primeros caracteres son la longitud. Hay que saltarlos 
                int numBytes = 16 + (posCaracter*8) + i;
                int fila = numBytes/bytesFila;
                int col = numBytes % (bytesFila) / 3; cadena[posCaracter] =(char)( cadena[posCaracter] | (imagen[fila][col][( (numBytes % bytesFila) % 3 )] & 1) << i) ;
            }
        }
    }
}